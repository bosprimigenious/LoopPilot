"""OpenAI-compatible API Model Adapter with injectable HTTP client for tests."""
...
class APIModelAdapter:
    ...
    def from_config(
        cls,
        adapter_id: str,
        config: APIModelConfig | dict[str, Any],
        artifact_dir: Path,
        http_client: HttpClient | None = None,
    ) -> APIModelAdapter:
    ...
    def healthcheck(self) -> dict[str, str]:
        return {"status": "configured", "adapter": self.adapter_id}
    ...
    def execute(
        ...
        try:
            response = self.http_client(payload, headers)
            ...
            artifact = self._write_artifact("api-error.txt", str(exc))
            return self._error(
                AdapterStatus.ERROR.value,
                ErrorCode.MODEL_OUTPUT_INVALID,
                start,
        ...
        transcript = self._write_artifact("transcript.json", json.dumps(response, sort_keys=True))
    ...
    def estimate_cost(self, _request: dict[str, Any]) -> dict[str, Any]:
        return {"cost": None}
