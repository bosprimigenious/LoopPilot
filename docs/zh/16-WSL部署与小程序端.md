...
| 审阅 | 待处理 review item 与只读详情 | mock，后续接 `/api/reviews`、`/api/reviews/{run_id}` |
...
`/api/summary/today` 返回今日 run 数、阻塞数、outcome 计数、最近运行和待审阅预览，供小程序首页只读展示。`/api/runs/{run_id}` 返回运行详情、`reportPath` 和 manifest 中的只读 `artifacts` 预览，供小程序运行详情页复制报告/产物路径。
