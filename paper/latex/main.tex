...
Table~\ref{tab:acceptance-measured} records a pinned local snapshot executed on 2026-07-16 for commit \texttt{03d9dae}.
The full hash, environment notes, commands, and artifact-bundle hash are archived in the paper experiment snapshot.
Newer repository badges must not be cited in this paper until the same gates are rerun and the commit hash is pinned.

\begin{table}[t]
  \centering
  \caption{Measured acceptance oracle results (2026-07-16; commit \texttt{03d9dae}).}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Unit + integration & \texttt{pytest -q} & 258 passed \\
    0.4 aggregate & \texttt{verify\_0\_4\_acceptance.py} & 11/11 READY \\
    PR \#8 oracles & \texttt{run\_failure\_injection\_bench.py --execute-oracles} & FI-1..FI-9 pass \\
    Artifact review & \texttt{build\_artifact\_review\_bundle.py} & sha256 pinned \\
    \bottomrule
  \end{tabular}
\end{table}
...
Table~\ref{tab:related-trust-objects} summarizes the trust-object distinction that organizes this section.
\looppilot{} does not compete with these lines on planning quality, action certificates, or enterprise policy coverage; it narrows the trust object to whether a \emph{terminal run} may be propagated as done.

\begin{table}[t]
  \centering
  \caption{Related-work contrast by trust object.}
  \label{tab:related-trust-objects}
  \small
  \begin{tabularx}{\linewidth}{@{}p{0.28\linewidth} X X@{}}
    \toprule
    \textbf{Line of work} & \textbf{Primary trust object} & \textbf{\looppilot{} contrast} \\
    \midrule
    Verify-gated completion & Completion claim checked by verifier & Terminal run admitted across review, evidence, recovery, and summaries \\
    Proof-carrying actions & Action certificate & Run-level evidence bundle and post-admission terminal truth \\
    Policy-constrained runtimes & Policy action / tool capability & Personal terminal truthfulness after safe actions have completed \\
    Software-engineering agents & Patch correctness / issue resolution & Whether an unapproved patch run may be called complete \\
    Artifact evaluation & Reproducible package & Per-run runtime bundle plus repository-level review bundle \\
    \bottomrule
  \end{tabularx}
\end{table}
